...
func Usage() {
	...
	fmt.Fprintf(os.Stderr, "\tpropetizer [flags] -type T [directory]\n")
	fmt.Fprintf(os.Stderr, "\tpropetizer [flags[ -type T files... # Must be a single package\n")
...
func main() {
	...
	g.Printf("// Code generated by \"propetizer%s\"; DO NOT EDIT\n", strings.Join(os.Args[1:], " "))
	g.Printf("\n")
...
func (g *Generator) buildAccessor(f Field, typeName string) {
	g.Printf("func (this *%s) Get%s() *%s {", typeName, f.name,
		f.typeName)
	g.Printf("  return this.%s", f.name)
	g.Printf("}")
	...
	for _, spec := range decl.Specs {
		...
		for _, fieldLine := range structType.Fields.List {
			for _, field := range fieldLine.Names {
				...
				//Get the underlying type string
				typeStr := fieldObj.Type().Underlying().String()
