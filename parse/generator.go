...
func (g *Generator) Run(path string, typeName string, outputName string, genFn GeneratorFunc) error {
	//Parse the package
	g.Prepare(path)
	...
	if outputName == "" {
		...
		outputName = filepath.Join(path, strings.ToLower(baseName))
	}
...
func (g *Generator) Prepare(path string) {

	if len(path) == 1 && isDirectory(path) {
		g.dir = path
		g.parsePackageDir(path)
	} else {
